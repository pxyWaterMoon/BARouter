...
class Gradient(BasePredictor):
    ...
    def get_policy(self,sample):
        distances, indices = self.KNN.kneighbors([sample])
        neighbor_labels = self.decision[indices[0]]
        # print(neighbor_labels)
        policy = np.zeros(len(self.action_list))
        for i in neighbor_labels:
            policy[i] += 1
        return policy/len(neighbor_labels)
    ...
    def take_action(self, sample):
        ...
        policy = self.get_policy(prompt_embedding)
        action_index = np.random.choice(len(self.action_list),p=policy)
