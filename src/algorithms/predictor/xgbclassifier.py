...
class XGBClassifierPredictor(BasePredictor):
    def __init__(self, key, SFT_dataset=None, buffer_size=128, offline = False):
        self.model = XGBClassifier(max_depth=4,learning_rate=0.01)
        self.key = key
    ...
    def sample2input(self, sample_list:list[dict], key = None):
        ...
        if key is not None:
            y = [1 if sample[key] > 0.5 else 0 for sample in sample_list]
            return X, np.array(y)
        return X
    ...
    def offline_training(self, dataset, key:str):
        ...
        for data in dataset:
            ...
            y.append(1 if data[key] > 0.5 else 0)
        ...
        print(f"The number of classes is {self.model.n_classes_}.")
    ...
    def online_update(self, sample, global_step):
        ...
        if len(self.buffer) >= self.buffer_size:
            ...
            # print(X.shape, y.shape)
            updated_model = XGBClassifier(max_depth=4)
