...
class K_means(BasePredictor):
    def __init__(self, dataset, key, k=5):
        ...
        for cluster_id in range(k):
            ...
            # 计算该聚类对应的y平均值
            self.sum[cluster_id] = np.sum(y[cluster_indices], axis=0)
        # print(self.sum)
        # print(self.counts)
        # exit(0)
    ...
    def online_update(self, sample, global_step):
        ...
        self.sum[label,action_idx] += value
    ...
    def predict(self, sample_list):
        ...
        beta_a = self.sum[label] + 1
        beta_b = self.counts[label] - self.sum[label] + 1
        # print(beta_a,beta_b)
        # exit(0)
        ret = np.random.beta(beta_a,beta_b)
        # ret = self.mean_val[label]
        # ucb = np.sqrt(1/self.counts[label]) * self.c
        # ret = ret + ucb if self.key=="reward" else ret - ucb
        return ret