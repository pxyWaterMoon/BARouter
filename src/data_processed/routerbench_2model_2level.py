...
save_path = "./data/processed/bert-base-uncased/routerbench_0shot_gpt4_llama270b_2level/"
...
for data in train_dataset:
    gt = data["gt"]
    if gt["meta/llama-2-70b-chat"]["reward"] > 0.5:
        level1_dataset.append(data)
    else:
        level2_dataset.append(data)
...
level1_df = pd.DataFrame(level1_dataset)
level1_df.to_parquet(os.path.join(save_path, "level1_online.parquet" if cost_scale == 1.0 else f"level1_online_cost_scale_{cost_scale}.parquet"), engine='pyarrow', index=False)
level2_df = pd.DataFrame(level2_dataset)
level2_df.to_parquet(os.path.join(save_path, "level2_online.parquet" if cost_scale == 1.0 else f"level2_online_cost_scale_{cost_scale}.parquet"), engine='pyarrow', index=False)
