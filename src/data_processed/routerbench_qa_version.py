...
save_path = "./data/processed/qa-version/routerbench_0shot/"
...
processed_dataset = []
tag = 0
# patten = "Please think step by step and given the final answer."
for index in tqdm(range(len(rawdata)), desc="Processing data"):
    ...
    processed_data = {
        "sample_id": sample_id,
        # "question": prompt[2:-10] + " " + patten,
        "question": prompt[2:-10],
        "answer": gt[2],
    }
    # if "mtbench-math" in sample_id and tag == 0:
    #     print(f"Processing sample {sample_id} with \n prompt: {prompt} \n gt: {gt}")
    #     tag = 1
    processed_dataset.append(processed_data)
print(f"The final number of dataset after processed is {len(processed_dataset)}")
...
all_df = pd.DataFrame(processed_dataset)
all_df.to_parquet(os.path.join(save_path, "all.parquet"), engine='pyarrow', index=False)
